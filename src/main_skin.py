...
from datasets.skin import get_ham10000, get_ph2
from networks import CENet
from utils.utils_skin import val, test, plot_result



parser = argparse.ArgumentParser()
parser.add_argument('--batch_size', type=int, default=20, help='batch_size per gpu')
parser.add_argument("--save_path", default="./model_pth/Skin") # Check the root Dir: SAVE path (checked)
parser.add_argument("--n_gpu", default=1)
parser.add_argument("--checkpoint", default=None)
parser.add_argument("--data_dir", default="/hdd/datasets/Skin/ISIC2018")
parser.add_argument('--input_channels', type=int, default=3, help='input channels of network input')
...
parser.add_argument('--freeze_bb', action='store_true', default=False, help='use this flag to freeze backbone')
...
parser.add_argument('--weights_fpath', type=str, default=None, help='path to the weights file for evaluation')
parser.add_argument('--out_merge_mode', type=str, default="cat", choices=["cat", "add"], help="cat or add")
parser.add_argument('--out_up_block', type=str, default="eucb", choices=["uprb", "eucb", "upcn", "uptc"], help="uprb: upsample with residual block, eucb: eca upsample block, upcn: upsample with conv block, uptc: upsample with transpose conv block")
...
vl_loader = DataLoader(db_val, batch_size=1, shuffle=True)
...
if args.eval:
    ...
    te_avg_dcs, te_avg_acc, te_avg_iou = test(net, te_loader, logging, -1)
    print(f"Skin dataset [{dataset_name}] -> Test <{args.tag}> -> Average Dice: {te_avg_dcs:.4f}, Average Accuracy: {te_avg_acc:.4f}, Average IoU: {te_avg_iou:.4f}")
    exit(0)
...
for epoch in range(0, args.max_epochs):    
    ...
    vl_avg_dcs = val(net, vl_loader, logging, best_dcs_vl, epoch+1)

    if vl_avg_dcs >= best_dcs_vl:
        te_avg_dcs, te_avg_acc, te_avg_iou = test(net, te_loader, logging, best_dcs_te)
    ...
    print(f"epoch:{epoch+1:03d}/{args.max_epochs}, loss:{train_loss/len(db_train):0.5f}, lr:{lr_:0.6f}, vl_DCS:{vl_avg_dcs*100:0.3f}, te_DCS:{te_avg_dcs*100:0.3f}")
